...
{
    if (argc < 2) {
        std::cerr << "usage: th2-archive-info [--list] ARCHIVE...\n";
        return 2;
    }

    try {
        const bool list = std::string_view(argv[1]) == "--list";
        for (int arg = list ? 2 : 1; arg < argc; ++arg) {
            ...
            const auto shown = list
                ? archive.entries().size()
                : std::min<std::size_t>(archive.entries().size(), 8);
